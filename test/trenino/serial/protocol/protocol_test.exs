defmodule Trenino.Serial.ProtocolTest do
  ...
  describe "IdentityRequest" do
    ...
    test "decode_body decodes valid message body" do
      # request_id (0x12345678 little endian) - no type byte
      body = <<0x78, 0x56, 0x34, 0x12>>
      {:ok, decoded} = IdentityRequest.decode_body(body)

      assert decoded == %IdentityRequest{request_id: 0x12345678}
    end

    test "decode_body returns error for incomplete message" do
      assert IdentityRequest.decode_body(<<0x78, 0x56>>) == {:error, :invalid_message}
    end
  ...
  describe "IdentityResponse" do
    ...
    test "decode_body decodes valid message body" do
      ...
      body = <<0x78, 0x56, 0x34, 0x12, 0x01, 0x02, 0x03, 0xEF, 0xBE, 0xAD, 0xDE>>
      {:ok, decoded} = IdentityResponse.decode_body(body)
    ...
    test "decode_body returns error for incomplete message" do
      assert IdentityResponse.decode_body(<<0x78, 0x56>>) == {:error, :invalid_message}
    end
  ...
  describe "Configure" do
    ...
    test "decode_body decodes valid message body" do
      # Body without type byte
      body = <<0x78, 0x56, 0x34, 0x12, 0x05, 0x02, 0x00, 0x0A, 0x64>>
    ...
    test "decode_body returns error for incomplete message" do
      assert Configure.decode_body(<<0x78, 0x56>>) == {:error, :invalid_message}
    end
  ...
  describe "Configure - Button type" do
    ...
    test "decode_body decodes button configuration" do
      # Body without type byte
      body = <<0x78, 0x56, 0x34, 0x12, 0x05, 0x02, 0x01, 0x0A, 0x32>>
      {:ok, decoded} = Configure.decode_body(body)
  ...
  describe "Configure - Matrix type" do
    ...
    test "decode_body decodes matrix configuration" do
      # Body without type byte
      body = <<0x78, 0x56, 0x34, 0x12, 0x03, 0x01, 0x02, 0x03, 0x02, 2, 3, 4, 5, 6>>
      {:ok, decoded} = Configure.decode_body(body)
    ...
    test "decode_body returns error for matrix with incorrect pin count" do
      # Says 3 rows + 2 cols = 5 pins, but only provides 4 bytes of pin data
      body = <<0x78, 0x56, 0x34, 0x12, 0x03, 0x01, 0x02, 0x03, 0x02, 2, 3, 4, 5>>
  ...
  describe "ConfigurationStored" do
    ...
    test "decode_body decodes valid message body" do
      # Body without type byte
      body = <<0x78, 0x56, 0x34, 0x12>>
      {:ok, decoded} = ConfigurationStored.decode_body(body)

      assert decoded == %ConfigurationStored{config_id: 0x12345678}
    end

    test "decode_body returns error for incomplete message" do
      assert ConfigurationStored.decode_body(<<0x78, 0x56>>) == {:error, :invalid_message}
    end
  ...
  describe "ConfigurationError" do
    ...
    test "decode_body decodes valid message body" do
      # Body without type byte
      body = <<0x78, 0x56, 0x34, 0x12>>
      {:ok, decoded} = ConfigurationError.decode_body(body)

      assert decoded == %ConfigurationError{config_id: 0x12345678}
    end

    test "decode_body returns error for incomplete message" do
      assert ConfigurationError.decode_body(<<0x78, 0x56>>) == {:error, :invalid_message}
    end
  ...
  describe "InputValue" do
    ...
    test "decode_body decodes positive value" do
      # Body without type byte
      body = <<0x0A, 0x34, 0x12>>
      {:ok, decoded} = InputValue.decode_body(body)

      assert decoded == %InputValue{pin: 0x0A, value: 0x1234}
    end

    test "decode_body decodes negative value" do
      # -1 in two's complement little endian: 0xFF, 0xFF
      body = <<0x0A, 0xFF, 0xFF>>
      {:ok, decoded} = InputValue.decode_body(body)
    ...
    test "decode_body returns error for incomplete message" do
      assert InputValue.decode_body(<<0x0A>>) == {:error, :invalid_message}
    end
  ...
  describe "Heartbeat" do
    ...
    test "decode_body decodes valid empty body" do
      # Heartbeat has no body
      {:ok, decoded} = Heartbeat.decode_body(<<>>)

      assert decoded == %Heartbeat{}
    end

    test "decode_body returns error for non-empty body" do
      # Heartbeat should have no body, extra bytes are invalid
      assert Heartbeat.decode_body(<<0x01>>) == {:error, :invalid_message}
    end
  ...
  describe "SetOutput" do
    ...
    test "decode_body decodes valid :high body" do
      # Body without type byte
      body = <<0x05, 0x01>>
      {:ok, decoded} = SetOutput.decode_body(body)

      assert decoded == %SetOutput{pin: 0x05, value: :high}
    end

    test "decode_body decodes valid :low body" do
      # Body without type byte
      body = <<0x05, 0x00>>
      {:ok, decoded} = SetOutput.decode_body(body)
    ...
    test "decode_body returns error for invalid value" do
      body = <<0x05, 0x02>>
      assert {:error, :invalid_message} = SetOutput.decode_body(body)
    end

    test "decode_body returns error for incomplete message" do
      assert SetOutput.decode_body(<<0x05>>) == {:error, :invalid_message}
    end
