defmodule Trenino.Firmware.UpdateCheckerTest do
  ...
  alias Trenino.Firmware.UpdateChecker
  alias Trenino.Repo
  ...
  describe "device-aware update notifications" do
    setup do
      ...
      case GenServer.whereis(Trenino.Serial.Connection) do
        nil ->
          {:ok, _pid} = start_supervised(Trenino.Serial.Connection)
    ...
    test "broadcasts update when device connects with older firmware" do
      ...
      Phoenix.PubSub.broadcast(Trenino.PubSub, "device_updates", {:devices_updated, [device]})
    ...
    test "dismisses notification when device with older firmware disconnects" do
      ...
      Phoenix.PubSub.broadcast(Trenino.PubSub, "device_updates", {:devices_updated, []})
    ...
    test "shows notification again when new device connects needing update" do
      ...
      Phoenix.PubSub.broadcast(Trenino.PubSub, "device_updates", {:devices_updated, [device1]})
      assert_receive {:firmware_update_available, "2.0.0"}, 1000
  ...
  defp build_test_device(version, status, port \\ "/dev/tty.test") do
    %Trenino.Serial.Connection.DeviceConnection{
      port: port,
      status: status,
