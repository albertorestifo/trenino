defmodule Trenino.Firmware.ReleaseManifestIntegrationTest do
  ...
  describe "end-to-end firmware release flow" do
    test "creates release from manifest and loads all devices" do
      ...
      {:ok, release} = Firmware.upsert_release(%{
        version: "2.2.1",
        tag_name: "v2.2.1",
      ...
      Enum.each(@test_manifest["devices"], fn device ->
        {:ok, _file} = Firmware.create_firmware_file(release.id, %{
          board_type: device["environment"],
          environment: device["environment"],
          download_url: "https://github.com/albertorestifo/trenino_firmware/releases/download/v2.2.1/#{device["firmwareFile"]}",
          file_size: 10_000
        })
      end)
    ...
    test "firmware file selection works with environment strings" do
      ...
      nano_file = Enum.find(release_with_files.firmware_files, fn file ->
      ...
      esp_file = Enum.find(release_with_files.firmware_files, fn file ->
