defmodule Trenino.Firmware.FilePath do
  @moduledoc """
  ...
  alias Trenino.Firmware.{BoardConfig, FirmwareFile, FirmwareRelease}

  @doc """
  Get the firmware cache directory.

  Uses `~/.trenino/firmware_cache` in production,
  `priv/firmware_cache` in development.
  """
  @spec cache_dir() :: String.t()
  def cache_dir do
    if Application.get_env(:trenino, :env) == :prod do
      Path.join([System.user_home!(), ".trenino", "firmware_cache"])
    else
      Application.app_dir(:trenino, "priv/firmware_cache")
    end
  end
