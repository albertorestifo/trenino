defmodule Mix.Tasks.VirtualJoystick do
  @moduledoc "Builds the bundled virtual joystick feeder sidecar."
  ...
  def run(args) do
    ...
    unless File.dir?(sidecar_dir),
      do: Mix.raise("virtual joystick directory not found at #{sidecar_dir}")
    ...
    case System.cmd("cargo", cargo_args, cd: sidecar_dir, into: IO.stream(:stdio, :line)) do
      ...
      {_, status} ->
        Mix.raise("Cargo build failed with exit code #{status}")
    end
  end
end
